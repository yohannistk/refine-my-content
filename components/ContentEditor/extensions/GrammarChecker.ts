...
declare module "@tiptap/core" {
  interface Commands<ReturnType> {
    grammarChecker: {
       ...
       */
      selection: (correction: Correction) => ReturnType;
...
let decorationSetForSelection: DecorationSet;
...
// (async () => {
//   console.log(await (db as any).ignoredWords.clear());
//   console.log(await (db as any).table("ignoredWords").toArray());
// })();
...
const makeSelection = (doc: PMNode, correction: Correction) => {
  const decorations: Decoration[] = [];

  const docFrom = correction.offset;
  const docTo = docFrom + correction.length;
    ...
    return {
      selection:
        ...
        ({ tr }) => {
          ...
          makeSelection(tr.doc, correction);
          return true;
        },
      proofread:
        (corrections) =>
